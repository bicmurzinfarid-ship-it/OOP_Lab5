...
public class LinkedListTabulatedFunction implements TabulatedFunction, Externalizable,Cloneable {
    ...
    public String toString() {
        ...
        FunctionNode current = this.head.getNext();
        while (current != head) {
            points += "(" + current.getData().getX() + ";" + current.getData().getY() + ")";
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof LinkedListTabulatedFunction) {
            ...
            FunctionNode thatCurrent = that.head.getNext();


            while (thisCurrent != this.head && thatCurrent != that.head) {
                if (Math.abs(thisCurrent.getData().getX() - thatCurrent.getData().getX()) > EPSILON ||
                        Math.abs(thisCurrent.getData().getY() - thatCurrent.getData().getY()) > EPSILON) {
                    return false;
                }
        ...
        if (obj instanceof TabulatedFunction) {
            TabulatedFunction that = (TabulatedFunction) obj;
            if (that.getPointsCount() != this.size) return false;
            int i = 0;
            FunctionNode Current = this.head.getNext();
            while (Current != head && i < size) {
                if (Math.abs(Current.getData().getX() - that.getPointX(i)) > EPSILON ||
                        Math.abs(Current.getData().getY() - that.getPointY(i)) > EPSILON) {
                    return false;
                }
                i++;
                Current = Current.getNext();
    ...
    public int hashCode() {
        ...
        while (current != this.head) {
            Hash = Hash * 31 + current.getData().hashCode();
            current = current.getNext();
        }
    ...
    public Object clone(){
        FunctionPoint array[] = new FunctionPoint[size];
        FunctionNode current = this.head.getNext();
        int i = 0;
        while(current!=head){
            array[i] = new FunctionPoint(
                    current.getData().getX(),
                    current.getData().getY()
            );
            current = current.getNext();
            i++;
        }
